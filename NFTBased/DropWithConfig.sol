...
contract ConfigMinter is ERC721Enumerable, Ownable {
    ...
    function setupSaleConfig(uint256 _timeOfDrop, uint256 _price, uint256 _maxMintAmount, uint256 _maxWallet) public onlyOwner{
        config.timeOfDrop = _timeOfDrop;
        config.price = _price;
        config.maxMintAmount = _maxMintAmount;
        config.maxWallet = _maxWallet;
    }
    ...
    function mint(uint256 _amnt) public payable {
        ...
        require(block.timestamp <= config.timeOfDrop);
        require(_amnt > 0, "Must mint at least one"); // check user doesnt try mint less than 1
        require(_amnt <= config.maxMintAmount, "You cannot mint this amount in one transaction"); // amnt per tx
        require(balanceOf(msg.sender) + _amnt <= maxPerWallet, "You are not allowed this many in one wallet");
        require(msg.value >= config.price, "User did not send enough ether with this tx"); // check they are paying enough
